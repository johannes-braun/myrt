...
vec3 sky(vec3 uv, vec3 cam, out vec3 sundir)
{
	...
	sundir = normalize(vec3(sx, sy, sz));
...
vec3 sky_noclouds(vec3 uv, vec3 cam, out vec3 sundir)
{
const float pi = 3.14159265359f;
	vec3 bottom = vec3(0.2f);
	vec3 top = vec3(0.3f, 0.5f, 1.f);
...
float cnoise(vec3 P, vec3 cam){
	P += vec3(sin(time) + 0.001*cam.x, 0, cos(time) + 0.001*cam.z);
