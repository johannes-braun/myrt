...
void forward_renderer::pass_gen_multidraw(scene& scene) {
  ...
  glUseProgram(m_gen_multidraw);
  glBindBufferBase(
      GL_SHADER_STORAGE_BUFFER, m_gen_multidraw_bindings.geometries, scene.get_scene_buffers().drawable_buffer);
...
void forward_renderer::create_resolve_shader(scene& scene) {
  ...
  mat_inject.info.replacement = scene.generate_glsl();
...
void forward_renderer::pass_resolve(scene& scene) {
  ...
  glBindBufferBase(GL_SHADER_STORAGE_BUFFER, m_resolve_bindings.materials, scene.get_scene_buffers().materials_buffer);
  glBindBufferBase(
...
void forward_renderer::pass_shadow(scene& scene) {
  ...
  rnu::mat4 view =
      inverse(rnu::rotation(rnu::quat(rnu::vec3(1, 0, 0), rnu::radians(-45))) * rnu::translation(rnu::vec3(0, 0, 15)));
