...
namespace myrt
{
  ...
  void pathtracer::reload_shaders(scene& scene) {
    m_compiling_program = m_program ? *m_program : 0u;
    m_is_compiling = true;
    std::jthread thread([&] {
      sf::Context context;
      glsp::preprocess_file_info vs_info;
      vs_info.file_path = res_dir / "../src/glsl/pathtracer.vert";
      ...
      fs_info.file_path = res_dir / "../src/glsl/pathtracer.frag";
      fs_info.definitions = {
          ...
          sdf_inject };
      fs_info.expand_in_macros = true; // sdf_inject may have preprocessor statements.
      const auto fragment_shader = glsp::preprocess_file(fs_info);

      const auto opt = make_program(vertex_shader.contents, fragment_shader.contents);
      m_compiling_program = opt ? *opt : 0u;
      m_is_compiling = false;
      });
    thread.detach();
  }

  bool pathtracer::is_compiling() const
  {
    return m_is_compiling;;
  }

  void pathtracer::bind_scene(scene& scene) const {
    glBindBufferBase(GL_SHADER_STORAGE_BUFFER, buffer_binding_bvh_nodes, scene.get_scene_buffers().bvh_nodes_buffer);
    glBindBufferBase(GL_SHADER_STORAGE_BUFFER, buffer_binding_bvh_indices, scene.get_scene_buffers().bvh_indices_buffer);
    glBindBufferBase(GL_SHADER_STORAGE_BUFFER, buffer_binding_indices, scene.get_scene_buffers().indices_buffer);
    glBindBufferBase(GL_SHADER_STORAGE_BUFFER, buffer_binding_vertices, scene.get_scene_buffers().vertices_buffer);
    glBindBufferBase(GL_SHADER_STORAGE_BUFFER, buffer_binding_normals, scene.get_scene_buffers().normals_buffer);
    glBindBufferBase(GL_SHADER_STORAGE_BUFFER, buffer_binding_drawables, scene.get_scene_buffers().drawable_buffer);
    glBindBufferBase(GL_SHADER_STORAGE_BUFFER, buffer_binding_global_bvh_nodes, scene.get_scene_buffers().global_bvh_nodes_buffer);
    glBindBufferBase(GL_SHADER_STORAGE_BUFFER, buffer_binding_global_bvh_indices, scene.get_scene_buffers().global_bvh_indices_buffer);
    glBindBufferBase(GL_SHADER_STORAGE_BUFFER, buffer_binding_materials, scene.get_scene_buffers().materials_buffer);
    glBindBufferBase(GL_SHADER_STORAGE_BUFFER, buffer_binding_sdf_data, scene.get_scene_buffers().sdf_data_buffer);
  ...
  void pathtracer::sample_internal(scene& scene, GLuint target_framebuffer, int width, int height)
  {
    ...
    auto const prepare_result = scene.prepare();
    bind_scene(scene);
    ...
    if (m_compiling_program != m_program)
    {
      if (m_program) glDeleteProgram(m_program.value());
      m_program = m_compiling_program;
      invalidate_counter();
    }
    ...
    glBlitNamedFramebuffer(m_framebuffer, target_framebuffer, 0, 0, width, height, 0, 0, width, height, GL_COLOR_BUFFER_BIT, GL_NEAREST);
  }
