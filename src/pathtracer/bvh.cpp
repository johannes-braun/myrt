...
[[nodiscard]] std::tuple<int, float, bool, int, int> bvh::compute_split_axis(
  ...
  for (auto index = node.first_child; index <= node.second_child; ++index) {
    ...
    rnu::vec3i const bin_id =
        rnu::clamp(rnu::vec3i(integral), rnu::vec3i(0), rnu::vec3i(bvh::binned_sah_bin_count - 1));
    for (auto i = 0; i < 3; ++i) {
...
std::vector<aabb_t> myrt::generate_triangle_bounds(
...
#define MYRT_INDEX_TYPE detail::uint
#define inout
#define out
#define MYRT_BVH_NODE_STRUCT  aligned_node_t const
#define MYRT_BVH_NODES_BUFFER m_nodes

#ifndef UINT_TYPE
#  define UINT_TYPE detail::uint
#endif

  /*bool MYRT_BVH_VISIT_PRIMITIVE(
      vec3 origin, vec3 direction, MYRT_INDEX_TYPE index, inout float max_ray_distance, out bool hits);
  bool MYRT_BVH_TRAVERSE(aligned_node_t* nodes, MYRT_INDEX_TYPE node_base_index, ray_t const& ray) {
    bool hits_primitive = false;

    UINT_TYPE visited_stack = 0;
    UINT_TYPE left_right_stack = 0;
    MYRT_INDEX_TYPE node_index = node_base_index;
    auto tmp = ray.intersect(MYRT_BVH_NODES_BUFFER[node_base_index].node.aabb());
    bool hits_any = tmp != std::nullopt;
    while (hits_any) {
      MYRT_BVH_NODE_STRUCT& current = MYRT_BVH_NODES_BUFFER[node_index];
      while (!(current.node.is_leaf())) {
        MYRT_BVH_NODE_STRUCT& first_child = MYRT_BVH_NODES_BUFFER[current.node.first_child + node_base_index];
        MYRT_BVH_NODE_STRUCT& second_child = MYRT_BVH_NODES_BUFFER[current.node.second_child + node_base_index];

        auto t_left = ray.intersect(first_child.node.aabb());
        auto t_right = ray.intersect(second_child.node.aabb());
      ...
      if ((current.node.is_leaf())) {
        for (MYRT_INDEX_TYPE tri = current.node.first_child; tri <= current.node.second_child; ++tri) {
          bool h = false;
          if (MYRT_BVH_VISIT_PRIMITIVE(ro, rd, MYRT_BVH_INDICES_BUFFER[primitive_base_index + tri], maxt, h))
            return true;
          if (h) {
            hits_primitive = true;
          }
      ...
      while ((visited_stack & UINT_TYPE(1)) != 1) {
        ...
        node_index = MYRT_BVH_NODES_BUFFER[node_index].node.parent() + node_base_index;
        visited_stack >>= UINT_TYPE(1);
        left_right_stack >>= UINT_TYPE(1);
      }
      node_index = node_base_index +
                   (((left_right_stack & 0x1) == 0x1)
                           ? MYRT_BVH_NODES_BUFFER[MYRT_BVH_NODES_BUFFER[node_index].node.parent() + node_base_index]
                                 .node.first_child
                           : MYRT_BVH_NODES_BUFFER[MYRT_BVH_NODES_BUFFER[node_index].node.parent() + node_base_index]
                                 .node.second_child);
      visited_stack ^= UINT_TYPE(1);
...
[[nodiscard]] std::experimental::generator<bvh::index_type> bvh::traverse(
  ...
  MYRT_INDEX_TYPE node_base_index = 0;
  UINT_TYPE visited_stack = 0;
  UINT_TYPE left_right_stack = 0;
  MYRT_INDEX_TYPE node_index = 0;
  auto tmp = should_traverse(MYRT_BVH_NODES_BUFFER[node_base_index].node.aabb());
  bool hits_any = tmp != std::nullopt;
  while (hits_any) {
    aligned_node_t current = MYRT_BVH_NODES_BUFFER[node_index];
    while (!(current.node.is_leaf())) {
      MYRT_BVH_NODE_STRUCT& first_child = MYRT_BVH_NODES_BUFFER[current.node.first_child + node_base_index];
      MYRT_BVH_NODE_STRUCT& second_child = MYRT_BVH_NODES_BUFFER[current.node.second_child + node_base_index];
  ...
  //const auto t_scene = should_traverse(m_nodes[node_index].node.aabb());
  ...
  //    const auto t_left = should_traverse(first.aabb());
  //    const auto t_right = should_traverse(second.aabb());

  //    if (!t_left.has_value() && !t_right.has_value())
  //      break;

  //    left_right_stack =
  //        (t_left.has_value() && t_right.has_value() && t_left.value() > t_right.value()) | (left_right_stack << 1);
  //    visited_stack = (t_left.has_value() && t_right.has_value()) | (visited_stack << 1);

  //    node_index = (!t_left.has_value() || (t_right.has_value() && (left_right_stack & 0x1))) ? current->second_child
  //                                                                                            : current->first_child;
  ...
  //  node_index = (left_right_stack & 0x1) ? m_nodes[m_nodes[node_index].node.parent()].node.first_child
