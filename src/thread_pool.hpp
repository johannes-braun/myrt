...
namespace myrt {
  class thread_pool {
  public:
    thread_pool(unsigned concurrency = std::thread::hardware_concurrency());
    ~thread_pool();

    template<typename Func>
    auto run_async(Func&& func) requires requires(Func&& func) { func(); }
    {
  ...
  concept async_loader_for = requires(Func func) { { func() }->std::convertible_to<Type>; };
  ...
  struct async_resource {
    ...
    async_resource(T&& value) requires std::movable<T> : m_value(std::move(value)) {

    }
    async_resource(T const& value) requires std::copyable<T> : m_value(value) {

    }

    std::experimental::generator<iterable_type const*> iterate() const requires iterable<T> {
      auto const lock = make_lock();
      for (iterable_type const& item : m_value)
        co_yield &item;
    }

    std::experimental::generator<iterable_type*> iterate() requires iterable<T> {
      auto const lock = make_lock();
      for (iterable_type& item : m_value)
        co_yield &item;
    }

    template<async_loader_for<T> Func>
    async_resource(thread_pool& pool, Func&& loader) {
      load_resource(pool, std::forward<Func>(loader));
    }

    template<typename ApplyFun>
    void current(ApplyFun&& apply) const requires requires(ApplyFun fun, T const& value) { fun(value); } {
      auto const lock = make_lock();
      apply(static_cast<T>(m_value));
    }

    T&& extract_current() requires std::movable<T> {
      auto const lock = make_lock();
    ...
    bool is_ready()
    {
      return !m_current_process.valid() || m_current_process.wait_for(std::chrono::seconds(0)) == std::future_status::ready;
    }

  private:

    auto make_lock() const {
